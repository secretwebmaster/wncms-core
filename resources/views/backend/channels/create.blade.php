...
<div class="card">
    <div class="card-header border-0 cursor-pointer px-3 px-md-9 bg-dark">
        <div class="card-title m-0">
            <h3 class="fw-bolder m-0 text-gray-100">{{ wncms()->getModelWord('channel', 'create') }}</h3>
        </div>
    </div>

    <div class="collapse show">
        <form class="form" method="POST" action="{{ route('channels.store') }}" enctype="multipart/form-data">
            @csrf
