...
@section('content')
    ...
    <div class="wncms-toolbar-filter mt-5">
        <form action="{{ route('body_parts.index') }}">
    ...
    <div class="wncms-toolbar-buttons mb-5">
        <div class="card-toolbar flex-row-fluid gap-1">
            ...
            @include('wncms::backend.common.default_toolbar_buttons', [
                'model_prefix' => 'body_parts',
            ])
        </div>
    </div>

    {{-- Index --}}
    @include('wncms::backend.common.showing_item_of_total', ['models' => $bodyParts])
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" value="1" data-model-id="{{ $bodyPart->id }}"/>
                                    </div>
                                </td>
                                {{-- Actions --}}
                                <td>
                                    <a class="btn btn-sm btn-dark fw-bold px-2 py-1" href="{{ route('body_parts.edit' , $bodyPart) }}">@lang('wncms::word.edit')</a>
                                    @include('wncms::backend.parts.modal_delete' , ['model'=>$bodyPart , 'route' => route('body_parts.destroy' , $bodyPart), 'btn_class' => 'px-2 py-1'])
                                </td>

                                {{-- Data --}}
                                <td>{{ $bodyPart->id }}</td>
                                <td>{{ $bodyPart->name }}</td>
                                <td>{{ $bodyPart->created_at }}</td>

                                @if(request()->show_detail)
                                <td>{{ $bodyPart->updated_at }}</td>
                                @endif
    ...
    {{-- Pagination --}}
    {{-- <div class="mt-5">
        {{ $bodyParts->withQueryString()->links() }}
    </div> --}}
