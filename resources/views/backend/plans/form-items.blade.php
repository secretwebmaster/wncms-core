                    ...
                    <div class="repeater-item d-flex mb-2 align-items-center">
                        ...
                        <select name="prices[{{ $key }}][duration_unit]" class="form-select form-select-sm me-2 w-auto">
                            ...
                            @foreach(['day', 'week', 'month', 'year'] as $unit)
                                <option value="{{ $unit }}" {{ $unit === old("prices.{$key}.duration_unit", $price['duration_unit']->value ?? null) ? 'selected' : '' }}>@lang('wncms::word.' . $unit)</option>
                            @endforeach
                        </select>

                        {{-- Price --}}
                        <input type="number" name="prices[{{ $key }}][price]" class="form-control form-control-sm me-2 w-auto" step="0.01" placeholder="@lang('wncms::word.price')" value="{{ old("prices.{$key}.price", $price['price'] ?? null) }}" required />
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            addRepeaterItem.addEventListener('click', () => {
                ...
                repeaterItem.innerHTML = `
                    ...
                    <input type="number" name="prices[${index}][price]" class="form-control form-control-sm me-2 w-auto" step="0.01" placeholder="@lang('wncms::word.price')" required />
                    <div class="form-check form-check-sm form-check-custom form-switch">
