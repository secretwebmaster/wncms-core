                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-item mb-3">
                                    ...
                                    @php
                                        $categories = wncms()
                                            ->tag()
                                            ->getList(['tag_type' => 'post_cateogory', 'cache' => false])
                                            ->map(function ($tag) {
                                ...
                                <div class="form-item mb-3">
                                    ...
                                    @php
                                        $tags = wncms()
                                            ->tag()
                                            ->getList(['tag_type' => 'post_tag', 'cache' => false])
                                            ->map(function ($tag) {
                                                return ['value' => $tag->id, 'name' => $tag->name];
                                            })
                                            ->toArray();
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary fw-bold"
                                    ...
                                    data-method="POST">@lang('wncms::word.submit')</button>
                            </div>
                        </div>
                    </form>
                    ...
                    <tbody id="table_with_checks" class="fw-semibold text-gray-600">
                        @foreach ($posts as $post)
                            <tr>
                                ...
                                <span title="{{ $website->domain }}"><a href="{{ $wncms->getRoute('frontend.posts.show', ['slug' => $post->slug], false, $website->domain) }}" target="_blank">#{{$website->id}}</a></span>
                                @endforeach
                            </td> --}}
                                ...
                                @if (request()->show_detail || request()->show_thumbnail)
                                    <td><img class="lazyload mw-80px rounded-3px" src="{{ $post->thumbnail }}" alt=""></td>
                                @endif

                                <td class="mw-400px text-truncate"><a href="{{ route('frontend.posts.show', ['slug' => $post->slug]) }}" target="_blank" title="{{ $post->title }}">{{ $post->title }}</a></td>
