    ...
    <div class="col-lg-9 fv-row">
        <select name="status" class="form-select form-select-sm" required>
            ...
            @foreach($statuses as $status)
                <option  value="{{ $status }}" {{ $status === ($banner->status ?? old('status', 'active')) ? 'selected' :'' }}><b>@lang('wncms::word.' . $status)</b></option>
            @endforeach
        </select>
    </div>
</div>
...
{{-- banner_thumbnail --}}
<div class="row mb-3">
    <label class="col-lg-3 col-form-label fw-bold fs-6" for="banner_thumbnail">@lang('wncms::word.thumbnail')</label>
    <div class="col-lg-9">
        ...
        <div class="form-text">@lang('wncms::word.allow_file_types', ['types' => 'png, jpg, jpeg, gif, webp, svg'])</div>
    </div>
</div>
...
{{-- Url --}}
<div class="row mb-1">
    <label class="col-lg-3 col-form-label fw-bold fs-6">@lang('wncms::word.url')</label>
    <div class="col-lg-9 fv-row">
...
{{-- contact --}}
<div class="row mb-1">
    <label class="col-lg-3 col-form-label fw-bold fs-6">@lang('wncms::word.contact')</label>
    <div class="col-lg-9 fv-row">
...
<div class="row mb-1">
    ...
    <div class="col-lg-9 fv-row">
        <input name="expired_at" value="{{ !empty($banner->expired_at) ? $banner->expired_at->format('Y-m-d') : '' }}" class="form-control form-control-sm" placeholder="@lang('wncms::word.choose_date_or_leave_blank')" id="kt_daterangepicker_3" />
    </div>
    <script>
            ...
            <div class="row w-100 m-0">
                @foreach($positions as $position)
                    <label class="col-12 col-md-3 form-check form-check-inline form-check-solid mx-0 mb-2">
                        ...
                        <span class="fw-bold ps-2 fs-6">@lang('wncms::word.' . $position)</span>
                    </label>
                @endforeach
            </div>
