...
final class ApiAuthSchema
{
    ...
    public static function createApiSecurityEvents(): void
    {
        ...
        Schema::create('api_security_events', function (Blueprint $table): void {
            ...
            $table->string('aggregate_key', 64)->nullable()->unique();
    ...
    /**
     * Create the hash-only recent-reauthentication proof table.
     */
    public static function createApiStepUpProofs(): void
    {
        if (self::hasCompatibleTable('api_step_up_proofs')) {
            return;
        }

        Schema::create('api_step_up_proofs', function (Blueprint $table): void {
            $table->id();
            $table->string('proof_id', 64)->unique();
            $table->string('proof_hash', 64)->unique();
            $table->foreignId('user_id')->constrained()->cascadeOnDelete();
            $table->foreignId('session_id')->constrained('api_sessions')->cascadeOnDelete();
            $table->json('purposes');
            $table->unsignedBigInteger('security_event_id')->default(0);
            $table->string('reservation_id', 64)->nullable()->index();
    ...
    /**
     * Create the hash-only atomic high-risk action plan table.
     */
    public static function createApiActionPlans(): void
    {
        if (self::hasCompatibleTable('api_action_plans')) {
            return;
        }

        Schema::create('api_action_plans', function (Blueprint $table): void {
            $table->id();
            $table->string('plan_id', 64)->unique();
            $table->string('confirmation_hash', 64)->unique();
            ...
            $table->string('effective_risk', 16);
            $table->string('reservation_id', 64)->nullable()->index();
            $table->timestamp('reserved_at')->nullable();
            $table->timestamp('expires_at')->index();
            $table->timestamp('consumed_at')->nullable()->index();
    ...
    private static function tableDefinitions(): array
    {
        ...
        return [
            ...
            'api_step_up_proofs' => [
                'primary_key' => ['id'],
                'columns' => ['id' => $column($integer, false), 'proof_id' => $column($string, false), 'proof_hash' => $column($string, false), 'user_id' => $column($integer, false), 'session_id' => $column($integer, false), 'purposes' => $column($json, false), 'security_event_id' => $column($integer, false), 'reservation_id' => $column($string, true), 'reserved_at' => $column($timestamp, true), 'expires_at' => $column($timestamp, false), 'consumed_at' => $column($timestamp, true), 'created_at' => $column($timestamp, true), 'updated_at' => $column($timestamp, true)],
                'indexes' => [['columns' => ['proof_id'], 'unique' => true], ['columns' => ['proof_hash'], 'unique' => true], ['columns' => ['reservation_id'], 'unique' => false], ['columns' => ['expires_at'], 'unique' => false], ['columns' => ['consumed_at'], 'unique' => false], ['columns' => ['user_id', 'session_id'], 'unique' => false]],
                'foreign_keys' => [['column' => 'user_id', 'table' => 'users'], ['column' => 'session_id', 'table' => 'api_sessions']],
            ],
            'api_action_plans' => [
                'primary_key' => ['id'],
