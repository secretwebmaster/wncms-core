...
| `authentication.access_token_expired` | `401` | Access token 或有明确期限的 service token 已过期 |
| `authentication.token_revoked` | `401` | Token 或 interactive session 已撤销 |
| `authorization.ability_denied` | `403` | Credential 未授予路由所需 ability |
| `authorization.permission_denied` | `403` | Actor 当前没有 WNCMS permission |
| `authorization.denied` | `403` | Legacy API gate 或 policy 拒绝请求 |
| `website.scope_missing` | `403` | 稳定的网站 selector 缺少或格式错误 |
| `website.scope_denied` | `403` | 明确指定的网站超出 token 或 actor scope |
...
| `idempotency.key_invalid` | `400` | Key 不是有效 UTF-8，或不在 `8..255` bytes 范围内 |
...
| `rate_limit.exceeded` | `429` | 超过已配置的 Laravel API rate limit |
...
| 404  | Not Found             | 资源未找到                 |
| 409  | Conflict              | 状态、revision 或 idempotency 冲突 |
| 429  | Too Many Requests     | 超过已配置的 API rate limit |
...
Client 必须依 stable `meta.error_code` 分支。重要边界包括 `risk.step_up_required`、`risk.plan_required`、`idempotency.key_missing`、`risk.credential_type_denied`、`security.audit_unavailable`、`authentication.legacy_disabled` 与 `authentication.refresh_reuse_detected`。参阅[安全策略](./security-policy.md)与[Legacy migration](./legacy-authentication.md)。
