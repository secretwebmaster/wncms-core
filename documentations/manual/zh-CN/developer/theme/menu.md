...
后台选单编辑器现在提供一个统一的 **Models** accordion。其下拉选单会根据继承 `Wncms\Models\BaseModel` 且声明 `public static bool $showInMenuEditor = true` 的模型自动产生。

默认流程：

- `BaseModel` 将 `showInMenuEditor` 预设为 `false`
- `Post`、`Page` 等 core 模型可自行覆写为 `true`
- 选单 manager 会先根据这些模型建立预设模型搜索来源
- 接着触发 `wncms.backend.menus.sources.resolve`，让插件或宿主专案继续调整最终来源列表

典型用途：

- 注册专案自订模型，例如 `Project`
- 通过关键字搜索记录，而不是一次载入整张表
- 将选中的记录加入选单，并只储存 `model_type` 与 `model_id`
- 选单标题留空时，执行期继续使用关联模型当前的 title / name
...
使用统一 **Models** accordion 时：

- 先在下拉选单选择模型
- 输入关键字搜索该模型资料表
- 点击结果，将其加入搜索框下方的已选列表
- 点击 **Add to Menu**，将这些已选记录转换为选单项目

**前台路由命名说明：**

- 首页连结请使用 `frontend.pages.home`（不要使用 `frontend.index`）。
- 完整前台路由清单请参考 [Frontend Routes](../route/frontend.md)。
- 若要了解路由定义细节，请查看 `routes/frontend.php`。
...
- 将项目巢状至父项目下以建立子选单
- 建议最多 3 层深度
- 父级项目会在项目列内显示折叠切换图示（`fa-caret-right` / `fa-caret-down`）用于展开与收合。
- 子层级排序在重新渲染后可保持稳定结构，仅更新 Nestable 根层列表。
- 当标签存在性 API 回传空值或非预期 `ids` 时，检查会安全降级，不会阻断菜单编辑器 UI 控件。
- 菜单项目编辑输入（弹窗与列表行内控件）现使用明确且唯一的 `id`/`name`，并确保 `label for` 对应正确，同时补齐 `autocomplete` 提示，以降低浏览器自动填充与可访问性警告。
