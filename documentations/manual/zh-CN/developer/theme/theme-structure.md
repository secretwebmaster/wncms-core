...
只有在以下必要结构都存在时，主题才会被视为可载入：

- `config.php`
- `views/`

`lang/` 与 `functions.php` 为可选项。

如果 public 主题目录存在但必要结构不完整，WNCMS 会将其视为无效并套用回退逻辑：

- 核心主题：回退到 `{WNCMS_RESOURCES_PATH}/themes/{themeId}` 内建主题
- 非核心主题：保持“主题未启用”行为
