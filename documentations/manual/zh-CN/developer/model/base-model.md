...
- 可翻译的模型名称解析
- 标签元资料定义支援
- 透过 `$showInMenuEditor` 控制后台选单编辑器可见性
...
`$showInMenuEditor` 用于控制后台选单编辑器是否应为该模型自动生成预设的可搜索来源。`BaseModel` 预设为 `false`，因此只有明确启用的模型才会出现在统一的 **Models** accordion 中。
...
`BaseModel` 提供集中式模式解析，避免在各处重复读取原始 config：
