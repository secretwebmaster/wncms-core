...
後台選單編輯器現在提供一個統一的 **Models** accordion。其下拉選單會根據繼承 `Wncms\Models\BaseModel` 且宣告 `public static bool $showInMenuEditor = true` 的模型自動產生。

預設流程：

- `BaseModel` 將 `showInMenuEditor` 預設為 `false`
- `Post`、`Page` 等 core 模型可自行覆寫為 `true`
- 選單 manager 會先根據這些模型建立預設模型搜尋來源
- 接著觸發 `wncms.backend.menus.sources.resolve`，讓插件或宿主專案繼續調整最終來源列表

典型用途：

- 註冊專案自訂模型，例如 `Project`
- 透過關鍵字搜尋紀錄，而不是一次載入整張表
- 將選中的紀錄加入選單，並只儲存 `model_type` 與 `model_id`
- 選單標題留空時，執行期繼續使用關聯模型當前的 title / name
...
使用統一 **Models** accordion 時：

- 先在下拉選單選擇模型
- 輸入關鍵字搜尋該模型資料表
- 點擊結果，將其加入搜尋框下方的已選列表
- 點擊 **Add to Menu**，將這些已選紀錄轉換為選單項目

**前台路由命名說明：**

- 首頁連結請使用 `frontend.pages.home`（不要使用 `frontend.index`）。
- 完整前台路由清單請參考 [Frontend Routes](../route/frontend.md)。
- 若要了解路由定義細節，請查看 `routes/frontend.php`。
