...
### テーマシステム
- ThemeServiceProvider にコアテーマ（default、starter、demo）のサポートを追加
- ThemeManager をリファクタリングし、メタデータ処理とアセット解決を改善
- テーマページ URL を解決する getThemePages() メソッドを追加
- テーマ翻訳のフォールバックメカニズムを改善
- PostController のビューパスをテーマ名前空間を使用して簡素化
- メタデータにテーマスクリーンショットサポートを追加

### ユーザーコントローラー
- ユーザーライフサイクル全体（ログイン、登録、ログアウトなど）にイベントフックを追加
- イベントを介して設定可能なオプションで登録フローをリファクタリング
- システム/テーマ設定をチェックする enabledRegistration() メソッドを追加
- テーマ名前空間構文を使用してビュー解決を改善
- クレジットシステムのイベントフックを追加（パッケージに移動）
- イベントサポートでダッシュボード、ログイン、プロフィールビューを強化
- カスタムユーザーページルーティングサポートを追加
- ユーザー投稿一覧機能を改善
- イベント駆動アーキテクチャによる関心の分離を改善

### サービスプロバイダー
- WNCMS 定数（ROOT、CONFIG_PATH、DATABASE_PATH など）を定義
- ファサード、エイリアス、設定を個別のメソッドに抽出
- テーマごとのアセットをサポートするようにテーマアセット公開を改善
- ビューコンポーザーを介してすべてのビューでエラー共有を追加
- app_log_level の代わりに log_level を使用するようにインストーラーを更新
- 専用メソッドによる関心の分離を改善
- コードの可読性と保守性を向上

### ルートサービスプロバイダー
- ルート登録で非推奨の namespace プロパティ使用を削除
- Laravel 8+ ルーティング規約に準拠

### 管理画面の改善
- メニュー編集機能を強化するため menu.js を追加
- メニュー編集ビューのレイアウトを更新
- 管理画面フッターと API 管理ビューを改善
- ウェブサイトコントローラーを更新
- 管理画面レイアウトを強化

### テーマのリファクタリング
- デフォルトテーマを resources/views/frontend/themes/default から resources/themes/default に移行
- より良い組織のためにテーマ構造を統合
- 古いテーマ設定ファイルを削除
- 非推奨のテーマアセットディレクトリを削除
- 古いフロントエンドテーマビューディレクトリを削除
- CreateThemeFile コマンドを削除（新しいテーマ構造に置き換え）

### その他
- インストーラーの log_level 設定を修正
- 繁体字中国語翻訳を更新
- インストーラーウィザード UI を改善
- インストールコマンドを更新
