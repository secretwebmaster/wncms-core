...
- Added JSON-ready `wncms:links:list` and `wncms:links:inspect` commands plus guarded `wncms:links:create`, `wncms:links:update`, and `wncms:links:delete` mutations.
...
- Improved backend “Install Default Theme” failure handling with permission-aware messaging (including actionable “Fix Permission” guidance) and clearer API status codes; advertisement position labels now use translated words when available.
...
- Fix broadcast, cache, and queue option names in Installer / Install commands
- Add Accept-Language Header toggle to system settings and inject it into runtime config
- Backend buttons now support success-action / fail-action with response and button context
- Fix Tagify losing values when manually entering tags
- Sidebar model list now supports MODEL_KEY constant to avoid undefined property errors
- Improve CreateModel command robustness to prevent overwriting existing models
- Refactor backend model route generation for consistent naming and permissions
- Fix CreateModelView template path issue to improve generation stability
