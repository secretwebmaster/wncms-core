...
| Links | Complete - CRUD, clone, bulk update, bulk sync tags, hookable form/index flow. | Partial - `wncms:links:list`, `wncms:links:inspect`, guarded `wncms:links:create`, guarded patch `wncms:links:update`, guarded `wncms:links:delete`, and atomic guarded `wncms:links:bulk-update` exist with JSON output; bulk tag automation is still missing. | Partial - v2 `links` resource and bridge actions exist; v1 links route/spec is not finalized. | Missing | Partial - link hook integration tests plus guarded create/update/delete/bulk-update dry-run, actor, scope, atomicity, audit, and hook-context tests exist; API v2 contract tests are still missing. | Partial - manager, event, user dashboard, and command docs include list/inspect/create/update/delete/bulk update; API links doc says no dedicated spec finalized. | Partial - `can:link_*`, cache flushes, manager website scoping support, hooks, and guarded create/update/delete/bulk-update CLI actor/permission/target-scope/audit paths exist; UI/API/MCP mutations still need consistent audit and scope enforcement. |
