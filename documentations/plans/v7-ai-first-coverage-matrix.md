...
| Links | Complete - CRUD, clone, bulk update, bulk sync tags, hookable form/index flow. | Partial - `wncms:links:list`, `wncms:links:inspect`, and guarded `wncms:links:create` exist with JSON output; update/delete/bulk automation commands are still missing. | Partial - v2 `links` resource and bridge actions exist; v1 links route/spec is not finalized. | Missing | Partial - link hook integration tests, read-only CLI tests, and guarded create dry-run/actor/audit tests exist; update/delete and API v2 contract tests are still missing. | Partial - manager, event, user dashboard, and command docs include list/inspect/create; API links doc says no dedicated spec finalized. | Partial - `can:link_*`, cache flushes, manager website scoping support, hooks, and create CLI actor/guard/audit write path exist; UI/API/MCP mutations still need consistent audit and scope enforcement. |
...
2. Build on the existing Link CLI commands (`list`, `inspect`, guarded `create`) and add update, delete, bulk update, and bulk sync tags.
